...
def solve(data: str):
    ...
    split = lambda t: map(lambda s: s.split(' '), t.split('\n'))
    wires = {x[:-1]: int(v) for x, v in split(wires)}
    gates = [(x, OP[op], y, z) for x, op, y, _, z in split(gates)]

    # Part 1
    Z = {g[-1] for g in gates if g[-1][0] == 'z'}
    ZZ = sorted(Z)[::-1]
    while Z:
        for x, op, y, z in gates:
            if x in wires and y in wires:
                wires[z] = op(wires[x], wires[y])
                Z.discard(z)
    print(int(''.join(str(wires[z]) for z in ZZ), 2))
    
    # Part 2: I fail miserably... 😭 At least slightly improve @lscddit's solution
    # https://www.reddit.com/r/adventofcode/comments/1hl698z/comment/m3kt1je/
    in_or = defaultdict(set)
    for x, op, y, _ in gates:
        in_or[op == or_] |= {x, y}
    wrong = [z for x, op, y, z in gates if op == and_ and z in in_or[0] and 'x00' not in [x, y]
